...
class WCGAN_GP(CGAN):
    ...
    def train_D(self, X_real, y_real: np.array):
        ...
        with tf.GradientTape() as tape:
            # * Get the predictions for real and fake data
            predictions_real = self.discriminator(
                tf.concat([X_real, y_real], axis=1)
            )  # D(X)
            predictions_fake = self.discriminator(
                tf.concat([fake_samples_without_labels, fake_labels], axis=1)
            )  # D(G(noise))

            # * Calculate the discriminator loss
            d_loss = self.discriminator_loss(predictions_real, predictions_fake)
